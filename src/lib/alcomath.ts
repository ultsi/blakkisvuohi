...
*/
export function getWidmarkFactorForUser(user) {
    const H = user.height
    const W = user.weight
...
export function getUserMetabolismRate() {
...
export function estimateAbsorbedAlcoholConcentration(user, milligrams, drinking_period) {
    const grams = milligrams / 1000
    const A = grams / MAGIC_NUMBER
    const r = getWidmarkFactorForUser(user)
...
export function estimateUnburnedAlcohol(user, ebac) {
    const r = getWidmarkFactorForUser(user)
    const half_t = 0.1066 // half life of absorption
    const t = 0.5
    const W = user.weight
...
export function calculateEBACFromDrinks(user, drinks) {
    const now = Date.now()
    const in30Mins = now + constants.HOUR_IN_MILLIS * 0.5
    const MR = getUserMetabolismRate()
    ...
    for (let i in drinks) {
        ...
        absorbedAlcoholConcentration += estimateAbsorbedAlcoholConcentration(user, drink.alcohol, absorption_time)
        absorbedAlcoholConcentrationIn30 += estimateAbsorbedAlcoholConcentration(user, drink.alcohol, absorption_time30)
    }
    ...
    let burn_hours = permilles30Min / getUserMetabolismRate()
    burn_hours = burn_hours > 0 ? burn_hours + 0.5 : burn_hours

    return {
        permilles: permilles,
        permilles30Min: permilles30Min,
        grams: estimateUnburnedAlcohol(user, permilles),
        burn_hours: burn_hours
    }
}

export function calculateEBACByHourFromDrinks(user, drinks, lastNHours, predictNHours) {
    const now = Date.now()
    ...
    for (let i = 0; i < lastNHours + predictNHours + 1; i += 1) {
        const time = new Date(startTime + constants.HOUR_IN_MILLIS * i)
        let absorbedAlcoholConcentration = 0
        let absorptionStartTime = Date.parse(drinks[0].created)
        for (let d in drinks) {
            const drink = drinks[d]
            const drinkTime: number = Date.parse(drink.created)
            const burn_time = (drinkTime - absorptionStartTime) / constants.HOUR_IN_MILLIS
            if (time.valueOf() - drinkTime > 0) {
                const ebac = absorbedAlcoholConcentration - MR * burn_time
                if (ebac <= 0) {
                    absorbedAlcoholConcentration = 0
                    absorptionStartTime = drinkTime
                }
                let absorption_time = (time.valueOf() - drinkTime) / constants.HOUR_IN_MILLIS
                absorbedAlcoholConcentration += estimateAbsorbedAlcoholConcentration(user, drink.alcohol, absorption_time)
            }
        }
        const permilles = Math.max(absorbedAlcoholConcentration - MR * Math.max((time.valueOf() - absorptionStartTime) / constants.HOUR_IN_MILLIS, 0), 0)
        permillesByHour[i] = {
            permilles: permilles,
            hour: time.getHours(),
            time: time
...
export function toStandardDrinks(alcohol_grams, decimals) {
    return utils.roundTo(alcohol_grams / constants.STANDARD_DRINK_GRAMS, decimals)
}

export function isAlcoholAmountOutOfLimits(mg) {
